...
class WP_Detect_Faces {
	...
	public function admin_scripts() {
		...
		wp_register_script( 'facedetection-ccv', FACE_DETECT_URL . '/js/ccv.js', array( 'jquery' ) );
		wp_register_script( 'facedetection-face', FACE_DETECT_URL . '/js/face.js', array( 'jquery', 'facedetection-ccv' ) );
		wp_register_script( 'jquery-facedetection', FACE_DETECT_URL . '/js/jquery.facedetection.js', array( 'facedetection-face' ) );
	...
	function edit_fields( $form_fields, $attachment ) {
		...
		if ( $faces )
			...
			$button .= '<button class="button face-detection-activate" type="button" data-attachment-id="' . $attachment->ID . '">' . __( 'Auto detect faces' ) . '</button>';

		$button .= '
				<span class="status"></span>
				<p class="description">' . __( 'If auto detect seems incorrect, use the manual control below to highlight any faces that were missed.' ) . '</p>
		...
		if ( $hotspots )
			$button .= '<button class="button add-hotspots has-hotspots" type="button" data-attachment-id="' . $attachment->ID . '">' . __( 'Manually detect faces' ) . '</button>';
		else
			$button .= '<button class="button add-hotspots" type="button" data-attachment-id="' . $attachment->ID . '">' . __( 'Add faces' ) . '</button>';

		$button .= '
				<span class="status"></span>
				<p class="description">' . __( 'Manually select the faces that you want to avoid cropping.' ) . '</p>';
